...
insert into calendar(id, events) values(0001, '{ "title" : "책읽기", "start" : "2023-07-20", "end": "2023-07-30" }');
...
CREATE TABLE tbl_temp_hb (
    hb_aid	VARCHAR2(5),
    hb_aname1	nVARCHAR2(30),
    hb_aname2	nVARCHAR2(30),
    hb_aname3	nVARCHAR2(30)
);

CREATE TABLE tbl_hobby (
    hb_seq	NUMBER		PRIMARY KEY,
    hb_aid	VARCHAR2(5)	NOT NULL,	
    hb_aname	nVARCHAR2(30)	NOT NULL
);
...
/*
    ...
    제1정규화가 된 취미 데이터는 취미1,취미2,취미3
    칼럼이 만들어진 table 이다.
    이테이블에 만약 4번째 취미를 추가하고 싶으면, 새로운 칼럼을
    추가하거나 4번째 취미를 다른 취미항목으로 교체 해야 한다.
    ...
    만약 취미항목이 3개 미만인 경우는 데이터가 저장되지 않는 칼럼이 발생하고
...
*/
SELECT hb_aid, hb_aname1 FROM tbl_temp_hb
UNION ALL
SELECT hb_aid, hb_aname2 FROM tbl_temp_hb
WHERE hb_aname2 IS NOT NULL
UNION ALL
SELECT hb_aid, hb_aname3 FROM tbl_temp_hb
WHERE hb_aname3 IS NOT NULL
ORDER BY hb_aid;
...
/*
    ...
    CREATE : 객체생성, 최초 프로젝트를 시작할때
    DROP : 객체 제거, 최초 프로젝트 시작할때 잘못 생성된 객체를 만들고자 할때
...
/*
    ...
    제2정규화가 되어있는 tbl_addr_hobby에는
    주소록의 ID(hb_id) 의 취미이름(hb_name)이 저장되어 있다
    hb_id 칼럼은 주소 테이블과 연계되어 있지만
    ...
    1. 기존의 Table 에서 일반 문자열로 되어 있는 부분을 때어 새로운 Table에 옮긴다.
    2. 기존 Table 과 새로운 Table 간에 Relation 설정을 한다.


*/

-- 1. tbl_hobby table 의 이름 변경 : Entity 을 Relation 으로 이름 변경
ALTER TABLE tbl_addrhobby
...
-- hb_name 칼럼의 데이터를 같은 데이터까지 그룹(묶음)지어 리스트를 만든다.
...
-- 다음을 진행할것
-- tbl_addr_hobby table 기존 table 을 DROP하고 새로운 구조로 생성하기
...
/*
    ...
    2,3 정규화 된 table 은 일반적으로 table 간의 join 이 되어야만 정보를
    명확히 알수 있다.
...
-- 주소,취미 Relation 과 주소 Entity 간의 참조무결성 검사
SELECT ah_aid, a_name, a_tel, a_addr, ah_hbcode
FROM tbl_addr_hobby
    LEFT JOIN tbl_address
        ON ah_aid = a_id
...
-- 주소,취미 Relation 과 취미 Entity 간의 참조무결성 검사
...
-- view 의 생성
-- 물리적 table 에 SQL 을 적용하여 작성하였더니 다소 복잡한 SQL 이 완성되었다.
-- 이후 이 SQL 을 사용할 일이 많을거 같다.
-- 그래서 이 SQL 을 view 로 생성하였다.
-- view는 일반적으로 Read Only(SELECT만 가능)
