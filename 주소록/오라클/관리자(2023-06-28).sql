...
-- Oracle 은 데이터를 저장하기 위하여 데이터 저장 공간을 먼저 설정해야 한다.
...
/*
    ...
    초기 크기는 1M(Byte) 이며 용량이 부족하면 1K(Byte) 씩 자동으로 증가 시켜라
*/
...
-- Oracle 12c 이상에서는 사용자 ID를 C##아이디 형식으로 생성해야 한다.
-- 사용자 ID를 관리하기가 상당히 불편하다.
-- 이 작업을 선행하지 않으면 사용자 ID 와 TableSpace 를 연동할 수 없다.
...
/*
    ...
    권한을 부여할때 로그인, CRUD하기, Table 생성하기 등의 권한을
    세부적으로 부여해야 하는데,
