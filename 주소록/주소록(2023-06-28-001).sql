...
-- 현재 tbl_address Talble 에 저장된 데이터가 모두 몇개이냐?
...
-- tbl_address 에서 A_ID 값의 최대값, 최소값 확인하기
...
/*
    ...
    ANSI SQL : 모든 DBMS SW 에 거의 공통으로 구현된 명령 문법
                표준화된 문법으로 DBMS SW 가 달라도 사용할수 있는 문법
...
-- 주소록 전체 데이터에서 a_id, a_name 속성(칼럼) 정보만 보여달라
...
-- 테이블의 모든 칼럼을 나열한 Projection
-- SELECT 명령에서 모든 칼럼(*) 으로 조회하는 것 보다는 ALL Projection 으로 조회를
-- 하는 것이 , 조회 속도면에서 더 유리하다.
-- 코딩(java, 등등) 과 연결하여 사용할때는 가급적 ALL Projection 을 사용하는 
-- 것이 좋다.
...
/*
    ...
    SELECTION : 특정 조건에 맞는 데이터를 SELECT 하는 것
...
-- a_tel 칼럼의 데이터를 5번째 위치 부터 잘라서 보여라
-- SUBSTR() : Oracle 의 문자열 자르기 함수
SELECT SUBSTR(a_tel, 5)
FROM tbl_address;

-- a_tel 칼럼의 데이터를 5번째 위치부터 4개만 잘라서 보여라 java 랑 다르다
SELECT SUBSTR(a_tel, 5, 4)
FROM tbl_address;

-- a_tel 칼럼의 데이터를 5번째 위치 부터 4개만 자르고
-- a_tel 칼럼의 기준으로 오름차순 정렬하여 보여라
SELECT SUBSTR(a_tel, 5, 4)
FROM tbl_address
ORDER BY a_tel;

/*
    1. tbl_address Table 이 있는지 확인
    2. 테이블이 있으면 데이터를 조회할 준비
    3. WHERE 절의 조건대로 데이터 SELECTION
    4. SELECT 절의 칼럼대로 데이터 PROJECTION
    5. ORDER BY 절의 칼럼을 기준으로 정렬
    
    SELECT FROM 은 필수 항목
    WHERE ORDER BY 는 선택 항목
*/
...
-- ID 값이 A0010 ~ A0020 인 데이터만 SELECTION 해라
SELECT *
FROM tbl_address
WHERE a_id >= 'A0010' AND a_id <= 'A0020' ;

/*
    SQL 에서 WHERE 등 조건을 부여 하여 SELECTION 할때
    특히 자중 조건을 부여 할때,
    조건문의 순서에 따라 SELECT 결과의 효율이 달라질 수 있다.
    조금더 깊이 생각하여 효율적인 SQL 명령문을 고민해 보자
    
    1. 첫번째 SELECTION 는 먼저 범위를 제한하여 개수를 제한하고
        전화번호를 찾는다.
        
    2. 두번째 SELECTION 는 전체 범위에서 전화번호를 찾고
